...
class AdminAreasFragment : Fragment() {
    ...
    private fun fetchData() {
        ...
        lifecycleScope.launch {
            val json = withContext(Dispatchers.IO) {
                try {
                    val req = Request.Builder().url("${getBaseUrl()}/api/customer/app/admin/areas")
            ...
            container.addView(TextView(ctx).apply { text = "📍 Area Wilayah"; setTextColor(Color.WHITE); textSize = 18f; typeface = Typeface.DEFAULT_BOLD; setPadding(0, 0, 0, 16) })
            val arr = json?.optJSONArray("data") ?: JSONArray()
            if (arr.length() == 0) { container.addView(TextView(ctx).apply { text = "Belum ada area terdaftar"; setTextColor(Color.parseColor("#94A3B8")); textSize = 14f; gravity = Gravity.CENTER; setPadding(0, 40, 0, 40) }); return@launch }
            for (i in 0 until arr.length()) {
                ...
                inner.addView(LinearLayout(ctx).apply {
                    ...
                    val desc = item.optString("description", ""); if (desc.isNotEmpty()) addView(TextView(ctx).apply { text = desc; setTextColor(Color.parseColor("#94A3B8")); textSize = 11f })
                })
                inner.addView(TextView(ctx).apply { text = "${item.optInt("customer_count", 0)} plg"; setTextColor(Color.parseColor("#38BDF8")); textSize = 13f; typeface = Typeface.DEFAULT_BOLD })
