...
class TechAttendanceFragment : Fragment() {
    ...
    private val takePictureLauncher = registerForActivityResult(
        ...
        if (bitmap != null) {
            ...
            val techName = CustomerApplication.sessionManager.getCustomerName().ifEmpty { "Teknisi Lapangan" }

            val watermarked = addGeotagWatermark(
                original = bitmap,
                company = dynamicCompanyName,
                roleTitle = "TEKNISI PORTAL - ABSENSI",
    ...
    private val cameraPermissionLauncher = registerForActivityResult(
        ...
        if (isGranted) {
            ...
            Toast.makeText(context, "Izin kamera diperlukan untuk mengambil foto selfie absensi", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun fetchAttendance() {
        ...
        lifecycleScope.launch {
            val json = withContext(Dispatchers.IO) {
                try {
                    val req = Request.Builder().url("${getBaseUrl()}/api/customer/app/tech/attendance/today")
            ...
            val today = json?.optJSONObject("data")

            contentContainer.addView(TextView(ctx).apply {
                text = "📍 Absensi Teknisi Lapangan"
                setTextColor(Color.WHITE); textSize = 18f; typeface = Typeface.DEFAULT_BOLD; setPadding(0, 0, 0, 4)
            })
            contentContainer.addView(TextView(ctx).apply {
                text = "Pencatatan kehadiran harian teknisi dengan foto selfie & Geo-Tagging posisi.";
            ...
            dynamicCompanyName = today?.optString("companyName", dynamicCompanyName) ?: dynamicCompanyName

            // Status Badge
            val badgeText = when (status) {
                "checked_in" -> "🟢 SEDANG BERTUGAS (CHECK-IN)"
                "checked_out" -> "🏁 SUDAH PULANG (CHECK-OUT)"
            ...
            inner.addView(TextView(ctx).apply {
                text = badgeText; setTextColor(Color.WHITE); textSize = 12.5f; typeface = Typeface.DEFAULT_BOLD
                setBackgroundColor(badgeColor); setPadding(18, 8, 18, 8); gravity = Gravity.CENTER
            })
            ...
            // Selfie Photo Section with Geo-tagging
            if (status == "none" || status == "checked_in") {
                ...
                val etNote = EditText(ctx).apply {
                    hint = "Catatan kehadiran / tugas hari ini (opsional)..."
                ...
                inner.addView(TextView(ctx).apply {
                    text = "✅ Anda telah menyelesaikan absensi hari ini. Terima kasih atas kerja keras Anda!"
    ...
    private fun submitAttendance(type: String, note: String) {
        val ctx = context ?: return
        val loc = getCurrentLocation()
        ...
        lifecycleScope.launch {
            val result = withContext(Dispatchers.IO) {
                try {
                    ...
                    val req = Request.Builder().url("${getBaseUrl()}/api/customer/app/tech/attendance/$type")
